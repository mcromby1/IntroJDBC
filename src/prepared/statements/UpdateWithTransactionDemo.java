 ...
 * combination of which is considered one large atomic transaction. By atomic we
 * mean that both sub-tasks must be completed successfully or the whole thing is
 * considered a failure. When that happens, the sub-tasks are rolled back,
 * meaning that the database is restored to its previous state.
 *
 * @author jlombardo
 */
